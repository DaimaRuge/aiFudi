...
- **Simple requirement**: 5-15 minutes
- **Medium complexity**: 15-30 minutes
- **PDF-based or complex requirements**: 30-60+ minutes
- Do not panic if generation takes a long time - OpenMAIC uses multiple AI agents working together

### Polling Strategy

- Conservative polling cadence: 60 seconds between polls
- Within a single turn: cap at 10 minutes of active polling
- If still running after 10 minutes: inform user and continue polling in next turn
- Never cancel or restart a running job unless explicitly requested

### Common Generation Failures

1. **Provider API key invalid**: Check `.env.local` configuration
2. **Model not configured**: Ensure `DEFAULT_MODEL` has correct provider prefix
3. **API rate limit exceeded**: Wait and retry later
4. **Out of memory**: Increase server memory or restart service
5. **Network timeout**: Check network connectivity to API providers

### Language Selection

If the user's requirement is in Chinese, explicitly specify language:
...
This ensures the generated classroom uses the correct language for all content.
